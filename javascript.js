...
let timerId;
let timerKey = 'JEE 02'; // Unique key for the 120-min timer
...
document.addEventListener('DOMContentLoaded', function () {
   ...
   // Function to generate URLs based on question number
    function generateUrl(section, questionNumber) {
        return `https://firebasestorage.googleapis.com/v0/b/mentorsmantratestportal1.appspot.com/o/${section}%2F${questionNumber}.png?alt=media`;
    }
    ...
    for (let i = 1; i <= 20; i++) {
        sectionData.phySec1.push({
            ...
            : generateUrl('ELECTROSTATICS', i),  // Default URL for other questions
            options
...
/*20*/ "C"  
    ...
    for (let i = 21; i <= 25; i++) {
        sectionData.phySec2.push({
            questionNumber: i,
            url: generateUrl('ELECTROSTATICS', i),
    ...
    for (let i = 1; i <= 20; i++) {
        sectionData.chemSec1.push({
            ...
            //  : i === 2 
            //    ? "https://firebasestorage.googleapis.com/v0/b/mentorsmantratestportal1.appspot.com/o/WPE%20%2B%20CIRCULAR%20MAINS%20TEST%2FCOM%20ADV%20QUES%2F1.jpeg?alt=media&token=e7d8978a-7339-4eb2-b791-bf7c5f205686"
            : generateUrl('MOLECONCEPT', i),  // Default URL for other questions
            options
...
function getAnswersData() {
    ...
    function updateQuestionDisplay() {
        ...
        if (currentSection === "phySec1" && sectionQuestionIndex[currentSection] === 30) { // Index starts from 0
            ...
            questionImage.style.width = "460px"; // Custom width
                ...
                questionImage.style.height = "450px"; // Custom height
                questionImage.style.width = "460px"; // Custom width
            }else if (currentSection === "phySec2" && sectionQuestionIndex[currentSection] === 4) { // Index starts from 0
                questionImage.style.height = "450px"; // Custom height
                questionImage.style.width = "460px"; // Custom width
                }else if (currentSection === "chemSec2" && sectionQuestionIndex[currentSection] === 2) { // Index starts from 0
                    questionImage.style.height = "450px"; // Custom height
                    questionImage.style.width = "460px"; // Custom width
                }else if (currentSection === "mathsSec2" && sectionQuestionIndex[currentSection] === 31) { // Index starts from 0
                    questionImage.style.height = "450px"; // Custom height
                    questionImage.style.width = "460px"; // Custom width
...
async function createPDFWithImages(userAnswers) {
    try {
        ...
        const imagesPHY = await fetchImagesFromFolder("ELECTROSTATICS");
        const imagesCHEM = await fetchImagesFromFolder("MOLECONCEPT");
        const imagesMATHS = await fetchImagesFromFolder("TRIGNOMETRY");
        ...
        for (let i = 0; i < imageUrls.length; i++) {
            ...
            if ([20, 21,22, 23, 24].includes(i) || [45, 46,48,49].includes(i) || [71, 72, 73, 74].includes(i)) {
                imageWidthPx = 500;
                imageHeightPx = 300;
            }
            ...
            const extraSpacing = (i === 23) ? pxToMm(60) : (i === 45) ? pxToMm(60) : 0;
